...
# sql connector
connection = pymysql.connect(host='localhost',user='root',password='Tinu@2002',db='cv')
...
# inserting Review data into user_Review table
def insertf_data(feed_name,feed_email,feed_score,comments,Timestamp):
    DBf_table_name = 'user_Review'
...
def run():
    ...
    img = Image.open('./Logo/Resume Analyzer picture.png')
    st.image(img)
    st.sidebar.markdown("# Choose Something...")
    activities = ["User", "Review", "About", "Admin"]
    choice = st.sidebar.selectbox("Choose among the given options:", activities)
    link = '<b>Built with  by <a href="https://dnoobnerd.netlify.app/" style="text-decoration: none; color: #021659;">Roshni & Tikeshwari</a></b>' 
    ...
    # Create table user_data and user_Review
    DB_table_name = 'user_data'
    ...
    ###### CODE FOR Review SIDE ######
        ...
        with st.form("my_form"):
            ...
            if submitted:
                ## Calling insertf_data to add dat into user Review
                insertf_data(feed_name,feed_email,feed_score,comments,Timestamp)    
                ## Success Message 
                st.success("Thanks! Your Review was recorded.") 
                ## On Successful Submit
                st.balloons()    


        # query to fetch data from user Review table
        ...
        cursor.execute('select feed_name, comments from user_Review')
    ...
    ###### CODE FOR ABOUT PAGE ######
        ...
        <p align="justify">
            ...
            <b>Review -</b> <br/>
            A place where user can suggest some Review about the tool.<br/><br/>
            <b>Admin -</b> <br/>
        ...
        <p align="justify">
            Built with  by 
            <a href="https://dnoobnerd.netlify.app/" style="text-decoration: none; color: grey;">Roshni & Tikeshwari</a> through 
    ...
    ###### CODE FOR ADMIN SIDE (ADMIN) ######
        ...
        if st.button('Login'):
            ...
            if ad_user == 'admin' and ad_password == 'admin@resume-analyzer':
                ...
                ### Fetch Review data from user_Review(table) and convert it into dataframe
                cursor.execute('''SELECT * from user_Review''')
                data = cursor.fetchall()

                st.header("**User's Review Data**")
                df = pd.DataFrame(data, columns=['ID', 'Name', 'Email', 'Review Score', 'Comments', 'Timestamp'])
                st.dataframe(df)

                ### query to fetch data from user_Review(table)
                query = 'select * from user_Review'
                plotfeed_data = pd.read_sql(query, connection)                        
